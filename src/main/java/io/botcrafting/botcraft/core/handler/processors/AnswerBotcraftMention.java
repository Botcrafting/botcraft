...
public class AnswerBotcraftMention implements MessageReplier  {
	...
	public boolean processMessage(Message message) {
		if (message.getLoweredText().contains(BOTCRAFT_NAME)
				...
				&& !message.getLoweredText().contains(QUESTION_WHERE_TO_GO)
				&& !message.getLoweredText().contains(QUESTION_STEPHEN_KING)
				&& !message.getLoweredText().contains(QUESTION_IS_GOING_TO_RAIN)
		) {
